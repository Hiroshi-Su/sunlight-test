...
import { sunPosition, lightOnScreen, resolveSite } from '../src/solar.js';

const fx = JSON.parse(readFileSync(new URL('./fixtures.json', import.meta.url)));
const site = {
  ...fx.site,
  rightAzimuth: (fx.site.facingAzimuth + 90) % 360,
};
const TOL_DEG = 0.03;
...
test('スクリーン座標変換がベクトル射影と一致する', () => {
  for (const c of fx.cases) {
    ...
    for (const k of ['x', 'y', 'z', 'windowIncidence']) {
...
test('config の解決（磁北→真北、窓の左右）', () => {
  const cfg = JSON.parse(readFileSync(new URL('../config/site.json', import.meta.url)));
  const base = cfg.sites[cfg.activeSite];
  const mk = (over) => ({ activeSite: 'x', sites: { x: { ...base, ...over } } });
  const r = resolveSite(mk({ screen: { facingAzimuth: 66, azimuthReference: 'magnetic', magneticDeclination: -7.5 }, window: { side: 'right', facingAzimuth: null } }));
  assert.equal(r.facingAzimuth, 58.5);
  assert.equal(r.windowAzimuth, 148.5);
  const l = resolveSite(mk({ screen: { facingAzimuth: 10, azimuthReference: 'true', magneticDeclination: -7.5 }, window: { side: 'left', facingAzimuth: null } }));
